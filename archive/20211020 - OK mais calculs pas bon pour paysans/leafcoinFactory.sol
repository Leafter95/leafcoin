   ...
   // }
    ...
    function mymint(address _address, uint256 _amount, address _client) public onlyOwner
    {
        ...
        for (uint i = 0; i < factories[_address].payees.length; i++) 
        {
            ...
            // On mine directement dans le portefeuille du beneficiaire
            if (cooperatives[factory.payees[i]].totalBring > 0)
                _dispatchToPeasants (factory.payees[i], amountTo);
            else
     ...
     * @param _amount amount of leafcoin mint allowance thanks to this delivery (representing  sequestrated carbon inside factory)
     */
    function _dispatchToPeasants (address _cooperative, uint256 _amount) private
    {
       
        
        require(cooperatives[_cooperative].totalBring > 0, "dispatchToPeasants: cooperative must call");
        require(_amount > 0, "dispatchToPeasants: amount = 0");
        ...
        for (uint i = 0; i < cooperatives[_cooperative].peasants.length; i++) 
        {
            ...
            cooperatives[_cooperative].brings[peasant] -= amountTo;
            
           
            _token.mint( peasant, amountTo );
